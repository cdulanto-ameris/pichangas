...
SELECT p.id,
    ...
    (count(DISTINCT s.partido_id) FILTER (WHERE pa.estado = 'cerrado'::estado_partido AND (s.equipo = 'blanco'::equipo_color AND pa.ganador = 'blanco'::resultado_partido OR s.equipo = 'negro'::equipo_color AND pa.ganador = 'negro'::resultado_partido)) * 3 + count(DISTINCT s.partido_id) FILTER (WHERE pa.estado = 'cerrado'::estado_partido AND (s.equipo = 'blanco'::equipo_color AND pa.ganador = 'negro'::resultado_partido OR s.equipo = 'negro'::equipo_color AND pa.ganador = 'blanco'::resultado_partido)) * 1)::integer AS puntos
   FROM profiles p
